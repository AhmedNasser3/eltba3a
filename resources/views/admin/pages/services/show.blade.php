            ...
            <div class="p-6 space-y-4 max-h-[500px] overflow-y-auto">
                @foreach ($orders as $order)
                    <div class="bg-gray-100 border-l-4 border-indigo-400 p-4 rounded-lg hover:bg-gray-200 transition duration-300">
                        ...
                        <div class="mt-4 flex gap-4">
                            <a href="{{ route('chats.show',['chat' => $order->id]) }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 transition duration-300">
                                محادثة 💬
                            </a>
                            <button onclick="updateOrderStatus({{ $order->id }}, 3)" class="bg-gray-600 text-white px-4 py-2 rounded-lg shadow hover:bg-gray-700 transition duration-300">
                                إنهاء الطلبية ✅
                            </button>
                        </div>
                    </div>
